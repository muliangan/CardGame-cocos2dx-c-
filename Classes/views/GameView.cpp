...
void GameView::initUI(PlayFieldView* playFieldView, StackView* stackView)
{
    ...
    CCLOG("GameView::initUI complete! Views and cards rendered.");
}
...
{
    CCLOG("GameView::playEntranceAnimation: Starting entrance animation");
    ...
    CCLOG("GameView::playEntranceAnimation: Entrance animation started for %d PlayField cards and %d Stack cards",
        static_cast<int>(_fieldCardViews.size()),
        static_cast<int>(_stackCardViews.size()));
}
...
std::map<int, CardPositionInfo> GameView::collectCardPositions() const
{
    ...
    CCLOG("GameView::collectCardPositions: Collected %d card positions", static_cast<int>(positions.size()));
    return positions;
}
...
void GameView::playUndoAnimation(CardView* cardView, const Vec2& targetWorldPos, bool isInStack)
{
    ...
    CCLOG("GameView::playUndoAnimation: Card ID %d moving to position (%.1f, %.1f) in %s",
          cardView->getModel() ? cardView->getModel()->getId() : -1,
          targetWorldPos.x, targetWorldPos.y,
          isInStack ? "Stack" : "PlayField");
}
