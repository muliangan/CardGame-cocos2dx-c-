...
bool GameModel::canMatch(CardModel* playfieldCard) const
{
    ...
    CardFaceType topRank = top->getRank();
    CardFaceType cardRank = playfieldCard->getRank();
    
    // 规则1：点数差为1即可匹配
    int topRankInt = static_cast<int>(topRank);
    int cardRankInt = static_cast<int>(cardRank);
    if (std::abs(topRankInt - cardRankInt) == 1) {
        return true;
    }
    
    // 规则2：Ace和King可以互相匹配
    bool isTopAce = (topRank == CFT_ACE);
    bool isTopKing = (topRank == CFT_KING);
    bool isCardAce = (cardRank == CFT_ACE);
    bool isCardKing = (cardRank == CFT_KING);
    
    if ((isTopAce && isCardKing) || (isTopKing && isCardAce)) {
        return true;
    }
